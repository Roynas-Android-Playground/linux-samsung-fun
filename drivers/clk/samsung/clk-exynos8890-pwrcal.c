...
#include "clk-exynos8890-pwrcal.h"
                                       ...
                                       struct clk_rate_request *req)
{
	...
	/* below minimum supported value, clamp to it */
	rate = rate_table[i - 1].rate;
			...
			unsigned long *best_parent_rate)
{
	...
	for (i = 1; i <= maxdiv; i++) {
		...
		parent_rate = clk_hw_round_rate(parent_hw,
				((rate * i) + i - 1));
		now = parent_rate / i;
	...
	if (!bestdiv) {
		...
		*best_parent_rate = clk_hw_round_rate(parent_hw, 1);
	}
					   ...
					   struct clk_rate_request *req)
{
	...
	div = samsung_divider_bestdiv(hw, req->rate, &parent_rate);
	if (div == 0) {
