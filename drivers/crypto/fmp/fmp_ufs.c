...
#define byte2word(b0, b1, b2, b3)                                 \
	((unsigned int)(b0) << 24) | ((unsigned int)(b1) << 16) | \
		((unsigned int)(b2) << 8) | (b3)
#define word_in(x, c)                                  \
	byte2word(((unsigned char *)(x) + 4 * (c))[0], \
...
int fmp_map_sg(struct ufshcd_sg_entry *prd_table, struct scatterlist *sg,
	       uint32_t sector_key, uint32_t idx, uint32_t sector,
	       struct bio *bio)
{
#if defined(CONFIG_FIPS_FMP)
	...
	if (sector_key & UFS_ENCRYPTION_SECTOR_BEGIN) {
		...
		if (disk_key_flag) {
			...
			if (fmp_xts_check_key(
				    bio->disk_key,
				    (uint8_t *)((uint64_t)bio->disk_key +
						FMP_KEY_SIZE),
				    FMP_KEY_SIZE)) {
				printk(KERN_ERR
				       "Fail to FMP XTS because enckey and twkey is the same\n");
				return -EINVAL;
			}

			if (disk_key_flag == 1)
				printk(KERN_INFO
				       "FMP disk encryption key is set\n");
			else if (disk_key_flag == 2)
				printk(KERN_INFO
				       "FMP disk encryption key is set after clear\n");
			ret = exynos_smc(SMC_CMD_FMP, FMP_KEY_SET, UFS_FMP, 0);
			if (ret < 0)
				panic("Fail to load FMP loadable firmware\n");
			else if (ret) {
				printk(KERN_ERR
				       "Fail to smc call for FMP key setting. ret = 0x%x\n",
				       ret);
				return ret;
			}
	...
	if (sector_key & UFS_FILE_ENCRYPTION_SECTOR_BEGIN) {
		...
		if (!strncmp(sg_page(sg)->mapping->alg, "aes", sizeof("aes")))
			aes_alg = AES_CBC;
		else if (!strncmp(sg_page(sg)->mapping->alg, "aesxts",
				  sizeof("aesxts"))) {
			aes_alg = AES_XTS;
#if defined(CONFIG_FIPS_FMP)
			if (prd_table[idx].size > 0x1000) {
				printk(KERN_ERR
				       "Fail to FMP XTS due to invalid size \
			...
			printk(KERN_ERR
			       "Invalid file encryption algorithm %s \n",
		...
		ret = file_enc_derive_iv(sg_page(sg)->mapping, index,
					 extent_iv);
		if (ret) {
			printk(KERN_ERR
			       "Error attemping to derive IV. ret = %c\n",
			       ret);
			return -EINVAL;
		}
		...
		for (j = 0; j < sg_page(sg)->mapping->key_length >> 2; j++)
			*(&prd_table[idx].file_enckey0 + j) =
				word_in(sg_page(sg)->mapping->key,
					(sg_page(sg)->mapping->key_length >>
					 2) - (j + 1));
#if defined(CONFIG_FIPS_FMP)
		if (!strncmp(sg_page(sg)->mapping->alg, "aesxts",
			     sizeof("aesxts"))) {
			if (fmp_xts_check_key(
				    (uint8_t *)&prd_table[idx].file_enckey0,
...
int fmp_map_sg_st(struct ufs_hba *hba, struct ufshcd_sg_entry *prd_table,
		  struct scatterlist *sg, uint32_t sector_key, uint32_t idx,
		  uint32_t sector)
{
	...
	if (hba->self_test_mode == XTS_MODE) {
		...
		if (fmp_xts_check_key((uint8_t *)&prd_table_st->file_enckey0,
				      (uint8_t *)&prd_table_st->file_twkey0,
				      prd_table_st->size)) {
			printk(KERN_ERR
			       "Fail to FMP XTS because enckey and twkey is the same\n");
			return -EINVAL;
		...
		printk(KERN_ERR
		       "Invalid algorithm for FMP FIPS validation. mode = %d\n",
		       hba->self_test_mode);
		return -EINVAL;
	}
	...
	if (sg_segments) {
		...
		scsi_for_each_sg(cmd, sg, sg_segments, i) {
			if (!((unsigned long)(sg_page(sg)->mapping) & 0x1) &&
				...
				print_hex_dump(
					KERN_ERR,
					"FIPS FMP descriptor before zeroize:",
					DUMP_PREFIX_NONE, 16, 1,
					&prd_table[i].file_iv0,
					sizeof(__le32) * 20, false);
#endif

				memset(&prd_table[i].file_iv0, 0x0,
				       sizeof(__le32) * 20);

#if FIPS_FMP_FUNC_TEST == 6 //Key Zeroization
				print_hex_dump(
					KERN_ERR,
					"FIPS FMP descriptor after zeroize:",
					DUMP_PREFIX_NONE, 16, 1,
					&prd_table[i].file_iv0,
					sizeof(__le32) * 20, false);
#endif
