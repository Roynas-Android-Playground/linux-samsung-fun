...
static const char *exynos_devfreq_rate_unit(struct exynos_devfreq_data *data)
{
	return data->devfreq_type == DEVFREQ_MIF ? "Hz" : "kHz";
}

#ifdef CONFIG_ARM_EXYNOS8890_DEVFREQ_DEBUG
					...
					struct device_attribute *attr, char *buf)
{
	...
	count = snprintf(buf, PAGE_SIZE, "%10u %s\n", get_freq,
			 exynos_devfreq_rate_unit(data));
				...
				struct device_attribute *attr, char *buf)
{
	...
	count += snprintf(buf, PAGE_SIZE, "%u%%, %u %s\n",
...
static int exynos_devfreq_parse_ect(struct exynos_devfreq_data *data, const char *dvfs_domain_name)
{
	...
	dvfs_block = exynos8890_ect_get_block(BLOCK_DVFS);
	if (dvfs_block == NULL)
		return -ENODEV;

	dvfs_domain = exynos8890_ect_dvfs_get_domain(dvfs_block, (char *)dvfs_domain_name);
	if (dvfs_domain == NULL)
		return -ENODEV;
	if (dvfs_domain->num_of_level <= 0 ||
	    (unsigned int)dvfs_domain->num_of_level >
		ARRAY_SIZE(data->opp_list))
		return -E2BIG;
...
static int exynos_devfreq_parse_dt(struct device_node *np, struct exynos_devfreq_data *data)
{
...
#if defined(CONFIG_EXYNOS8890_ECT)
	/* Native MIF is unit-safe in DT; the legacy ECT levels are in kHz. */
	if (data->devfreq_type != DEVFREQ_MIF) {
		if (of_property_read_string(np, "devfreq_domain_name",
					    &devfreq_domain_name))
			return -ENODEV;
		not_using_ect = exynos_devfreq_parse_ect(data,
						     devfreq_domain_name);
	}
#endif
	...
	if (not_using_ect) {
		if (of_property_read_u32(np, "opp_list_length", &data->opp_list_length))
			return -ENODEV;
		if (!data->opp_list_length ||
		    data->opp_list_length > ARRAY_SIZE(data->opp_list))
			return -E2BIG;
	...
	} else if (!strcmp(data->governor_name, "powersave")) {
		data->gov_type = POWERSAVE;
	} else if (!strcmp(data->governor_name, "performance")) {
		data->gov_type = PERFORMANCE;
	} else {
...
{
	if ((unsigned int)type >= DEVFREQ_TYPE_END) {
...
u32 get_target_devfreq_rate(enum exynos_devfreq_type type, char *name, u32 freq)
{
	...
	if (type < 0 || type >= DEVFREQ_TYPE_END)
		return target_rate;
	...
	if (data->ops.get_target_freq)
		target_rate = data->ops.get_target_freq(name, freq);

	dev_dbg(data->dev, "%s(%d): request freq: %u, target_freq: %u\n",
...
int exynos_devfreq_sync_voltage(enum exynos_devfreq_type type, bool turn_on)
{
	...
	if (turn_on) {
		if (!data->vdd) {
			...
			if (ret) {
				dev_err(data->dev,
					"failed sync voltage (%d:%lu %s:%u uV)\n",
					type, freq, exynos_devfreq_rate_unit(data),
					data->new_volt);
				goto out;
			}
...
static int exynos_init_freq_table(struct device *dev, struct exynos_devfreq_data *data)
{
	...
	for (i = 0; i < data->max_state; i++) {
		...
		if (ret) {
			dev_err(dev, "failed to add OPP %u %s\n", freq,
				exynos_devfreq_rate_unit(data));
			return ret;
		} else {
			dev_info(dev, "DEVFREQ: %8u %s, %8u uV\n", freq,
				 exynos_devfreq_rate_unit(data), volt);
		}
	}
				...
				unsigned long *target_freq, u32 flags)
{
	...
	struct exynos_devfreq_data *data = platform_get_drvdata(pdev);
	ktime_t before_target, after_target, before_setfreq, after_setfreq;
	struct dev_pm_opp *target_opp;
	...
	/*
	 * Governor start may evaluate a target before domain initialization is
	 * complete. Treat that deliberately suppressed transition as success.
	 */
	if (data->devfreq_disabled) {
		*target_freq = data->devfreq_profile.initial_freq;
		return 0;
	}

	before_target = ktime_get();
	...
	*target_freq = dev_pm_opp_get_freq(target_opp);
	...
	dev_dbg(dev, "LV_%d %u %s %u uV => LV_%d %u %s %u uV\n",
		data->old_idx, data->old_freq, exynos_devfreq_rate_unit(data),
		data->old_volt, data->new_idx, data->new_freq,
		exynos_devfreq_rate_unit(data), data->new_volt);
	...
	if (data->use_switch_clk) {
		...
		if (data->ops.set_freq) {
			ret = data->ops.set_freq(dev, data->old_freq,
					data->new_freq, data);
			if (ret) {
				dev_err(dev, "failed set frequency (%u -> %u %s)\n",
		...
		after_setfreq = ktime_get();

		setfreq_time = ktime_us_delta(after_setfreq, before_setfreq);
		...
		if (data->set_volt_order == PRE_SET_VOLT) {
			...
			if (ret) {
				dev_err(dev,
					"failed set voltage (%u %s:%u uV -> %u %s:%u uV)\n",
					data->old_freq,
					exynos_devfreq_rate_unit(data), data->old_volt,
					data->new_freq,
					exynos_devfreq_rate_unit(data), data->new_volt);
				goto out;
			}
...
out:
	...
	after_target = ktime_get();

	target_time = ktime_us_delta(after_target, before_target);
...
static int exynos_devfreq_suspend(struct device *dev)
{
	...
	if (ret) {
		dev_err(dev, "failed to suspend devfreq\n");
		return ret;
	}
...
static int exynos_devfreq_resume(struct device *dev)
{
	...
	return devfreq_resume_device(data->devfreq);
}
...
static int exynos_devfreq_probe(struct platform_device *pdev)
{
	...
	if (data->use_regulator) {
		...
		if (ret) {
			dev_err(data->dev,
				"failed set voltage in probe (%u %s:%u uV)\n",
				data->old_freq, exynos_devfreq_rate_unit(data),
				data->new_volt);
			goto err_set_voltage;
		}
	} else {
		/* A domain-specific owner may serialize its own regulator. */
		data->new_volt = data->opp_list[data->old_idx].volt;
	}
	...
	data->devfreq->suspend_freq = data->suspend_freq;
	if (data->devfreq_type == DEVFREQ_MIF &&
	    data->gov_type == PERFORMANCE)
		data->devfreq->resume_freq = data->max_freq;
	...
	/*
	 * DEVFREQ_GOV_START ran while target() was intentionally suppressed.
	 * MIF's fixed-high bring-up policy therefore needs one real update after
	 * transactions become available; otherwise it can remain at boot rate.
	 */
	if (data->devfreq_type == DEVFREQ_MIF &&
	    data->gov_type == PERFORMANCE) {
		...
		if (ret) {
			dev_err(data->dev,
				"failed to apply initial MIF performance target: %d\n",
				ret);
			data->devfreq_disabled = true;
			goto err_initial_target;
		}
	}

	mutex_lock(&devfreq_data_lock);
	if (devfreq_data[data->devfreq_type]) {
		mutex_unlock(&devfreq_data_lock);
		ret = -EBUSY;
		data->devfreq_disabled = true;
		goto err_initial_target;
...
err_init_table:
	dev_pm_opp_remove_all_dynamic(data->dev);
...
static void exynos_devfreq_remove(struct platform_device *pdev)
{
	...
	mutex_lock(&devfreq_data_lock);
	if (devfreq_data[data->devfreq_type] == data)
		devfreq_data[data->devfreq_type] = NULL;
	mutex_unlock(&devfreq_data_lock);
...
static const struct of_device_id exynos_devfreq_match[] = {
	{
		.compatible	= "samsung,exynos8890-mif-devfreq",
	},
	{},
};
