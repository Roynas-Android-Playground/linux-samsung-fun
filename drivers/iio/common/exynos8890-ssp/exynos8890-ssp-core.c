 ...
 *
 * SSP commands require a runtime patch and a firmware-ready indication.
 * No firmware-service interface is implemented, so probe only registers
 * the receive consumer and never sends WHOAMI or sensor commands.
 */
...
static int exynos8890_ssp_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "registered; firmware readiness interface unavailable\n");
