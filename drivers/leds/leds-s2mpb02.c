...
static int s2mpb02_led_setup(struct s2mpb02_led_data *led_data)
{
	...
	ret |= s2mpb02_update_reg(led_data->i2c, S2MPB02_REG_FLED_TIME1,
				  data->timeout << leds_shift[id],
				  leds_mask[id]);

	value = s2mpb02_led_get_en_value(led_data, 0);
	ret = s2mpb02_update_reg(led_data->i2c, S2MPB02_REG_FLED_CTRL1, value,
				 S2MPB02_FLED_ENABLE_MODE_MASK);
	...
	ret |= s2mpb02_update_reg(led_data->i2c, S2MPB02_REG_FLED_CUR2,
				  data->brightness << leds_shift[id],
				  leds_mask[id]);
	ret |= s2mpb02_update_reg(led_data->i2c, S2MPB02_REG_FLED_TIME2, 0x00,
				  leds_mask[id]);

	value = s2mpb02_led_get_en_value(led_data, 0);
	ret = s2mpb02_update_reg(led_data->i2c, S2MPB02_REG_FLED_CTRL2, value,
				 S2MPB02_FLED2_ENABLE_MODE_MASK);
#endif

	if (data->irda_off) {
		ret = s2mpb02_update_reg(led_data->i2c, S2MPB02_REG_FLED_CTRL2,
					 0x04, S2MPB02_TORCH_MASK);
	}
...
static void s2mpb02_led_get_status(struct led_classdev *led_cdev, bool status,
				   bool onoff)
{
	int ret = 0;
	u8 value[6] = {
	...
	ret = s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_CTRL1,
			       &value[0]); //Fled_ctrl1
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_CTRL2,
				&value[1]); //Fled_ctrl2
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_CUR1,
				&value[2]); //Fled_cur1
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_TIME1,
				&value[3]); //Fled_time1
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_CUR2,
				&value[4]); //Fled_cur2
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_TIME2,
				&value[5]); //Fled_time2
	if (unlikely(ret < 0)) {
	...
	printk("%s[%d, %d] : Fled_ctrl1 = 0x%12x, Fled_ctrl2 = 0x%13x, Fled_cur1 = 0x%14x, "
	       ...
	       __func__, status, onoff, value[0], value[1], value[2], value[3],
	       value[4], value[5]);
}
...
int s2mpb02_set_torch_current(bool torch_mode)
{
	...
	data->brightness = torch_mode ? S2MPB02_TORCH_OUT_I_60MA :
					original_brightness[data->id];

	/* set current */
	ret = s2mpb02_set_bits(led_data->i2c, S2MPB02_REG_FLED_CUR1,
			       leds_mask[data->id],
			       data->brightness << leds_shift[data->id]);
	if (unlikely(ret)) {
...
static ssize_t s2mpb02_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	...
	if (value == 0) {
		...
		global_led_datas[S2MPB02_TORCH_LED_1]->data->brightness =
			global_led_datas[S2MPB02_TORCH_LED_1]
				->data->torch_current_value;
		led_set(global_led_datas[S2MPB02_TORCH_LED_1]);
	} else if (value == 100) {
		/* Factory mode Turn on Torch */
		global_led_datas[S2MPB02_TORCH_LED_1]->data->brightness =
			global_led_datas[S2MPB02_TORCH_LED_1]
				->data->factory_torch_current_value;
		led_set(global_led_datas[S2MPB02_TORCH_LED_1]);
	...
	if (value <= 0) {
		s2mpb02_set_bits(
			global_led_datas[S2MPB02_TORCH_LED_1]->i2c,
			S2MPB02_REG_FLED_CUR1,
			leds_mask[global_led_datas[S2MPB02_TORCH_LED_1]
					  ->data->id],
			original_brightness[S2MPB02_TORCH_LED_1] << leds_shift
					[global_led_datas[S2MPB02_TORCH_LED_1]
						 ->data->id]);
		global_led_datas[S2MPB02_TORCH_LED_1]->data->brightness =
...
#ifdef CONFIG_LEDS_S2MPB02_MULTI_TORCH_REAR2
ssize_t s2mpb02_store_2nd(struct device *dev, struct device_attribute *attr,
			  const char *buf, size_t count)
{
	int value = 0;
	...
	if (value == 0) {
		...
		/* Turn on Torch */
		global_led_datas[S2MPB02_TORCH_LED_2]->data->brightness =
			global_led_datas[S2MPB02_TORCH_LED_2]
				->data->torch_current_value;
		led_set(global_led_datas[S2MPB02_TORCH_LED_2]);
	} else if (value == 100) {
		/* Factory mode Turn on Torch */
	...
	if (value <= 0) {
		s2mpb02_set_bits(
			global_led_datas[S2MPB02_TORCH_LED_2]->i2c,
			S2MPB02_REG_FLED_CUR2,
			leds_mask[global_led_datas[S2MPB02_TORCH_LED_2]
					  ->data->id],
			original_brightness[S2MPB02_TORCH_LED_2] << leds_shift
					[global_led_datas[S2MPB02_TORCH_LED_2]
						 ->data->id]);
		global_led_datas[S2MPB02_TORCH_LED_2]->data->brightness =
...
static ssize_t s2mpb02_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
	if (global_led_datas[S2MPB02_TORCH_LED_1] == NULL) {
		pr_err("<%s> global_led_datas[S2MPB02_TORCH_LED_1] is NULL\n",
				  ...
				  const char *buf, size_t count)
{
	...
	if (value == 1) {
		...
		ret |= s2mpb02_update_reg(
			global_led_datas[0]->i2c, S2MPB02_REG_FLED_TIME2,
		...
		ret |= s2mpb02_update_reg(global_led_datas[0]->i2c,
					  S2MPB02_REG_FLED_CUR2, 0x0E,
					  S2MPB02_FLED_CUR2_TORCH_CUR2_MASK);
	} else if (value == 0) {
	...
	ret |= s2mpb02_read_reg(global_led_datas[0]->i2c,
				S2MPB02_REG_FLED_CTRL2, &ledvalue);
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_TIME2,
				&value[5]); //Fled_time2
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_IRON1,
				&value[6]); //Fled_iron1
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_IRON2,
				&value[7]); //Fled_iron2
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_IRD1,
				&value[8]); //Fled_ird1
	ret |= s2mpb02_read_reg(led_data->i2c, S2MPB02_REG_FLED_IRD2,
				&value[9]); //Fled_ird2
	if (unlikely(ret < 0)) {
		printk("%s : error to get dt node\n", __func__);
	}
...
static DEVICE_ATTR(rear_torch_flash,
...
static DEVICE_ATTR(rear_torch_flash2,
		   S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH,
		   s2mpb02_show, s2mpb02_store_2nd);
#endif
...
int s2mpb02_ir_led_current(uint32_t current_value)
{
	...
	ret = s2mpb02_write_reg(global_led_datas[0]->i2c, S2MPB02_REG_FLED_CUR2,
				data);
	if (ret < 0)
...
int s2mpb02_ir_led_pulse_width(uint32_t width)
{
	...
	/* set 0x18, 0x19 */
	ret |= s2mpb02_write_reg(global_led_datas[0]->i2c,
				 S2MPB02_REG_FLED_IRON1, iron1);
	ret |= s2mpb02_set_bits(global_led_datas[0]->i2c,
...
int s2mpb02_ir_led_pulse_delay(uint32_t delay)
{
	...
	ird1 = (value >> 2) & 0xFF;
	ird2 = ((value & 0x03) << 6) |
	       0x2C; /* value 0x2C means RSVD[5:0] Reserved */

	pr_info("[%s] IRD1(0x%02x), IRD2(0x%02x)\n", __func__, ird1, ird2);

	/* set 0x1A, 0x1B */
	ret |= s2mpb02_write_reg(global_led_datas[0]->i2c,
				 S2MPB02_REG_FLED_IRD1, ird1);
	ret |= s2mpb02_write_reg(global_led_datas[0]->i2c,
				 S2MPB02_REG_FLED_IRD2, ird2);
	if (ret < 0)
		pr_err("[%s] i2c write error", __func__);
			       ...
			       struct s2mpb02_led_platform_data *pdata)
{
	...
	for_each_child_of_node(np, c_np) {
		...
		pdata->leds[index].use_torch_current_value =
			of_property_read_bool(c_np, "use_torch_current_value");
		if (pdata->leds[index].use_torch_current_value) {
			ret = of_property_read_u32(c_np, "torch_current_value",
						   &temp);
			if (ret) {
			...
			ret = of_property_read_u32(
				c_np, "factory_torch_current_value", &temp);
			if (ret) {
			...
			if (index != S2MPB02_FLASH_LED_1) {
				...
				pdata->leds[index].torch_current_value =
					S2MPB02_TORCH_OUT_I_60MA;
#else
				pdata->leds[index].torch_current_value =
					S2MPB02_TORCH_OUT_I_240MA;
#endif
				pdata->leds[index].factory_torch_current_value =
					S2MPB02_TORCH_OUT_I_240MA;
			}
		}
...
static int s2mpb02_led_probe(struct platform_device *pdev)
{
...
#endif
	led_datas = kzalloc(sizeof(struct s2mpb02_led_data *) * S2MPB02_LED_MAX,
			    GFP_KERNEL);
	...
	for (i = 0; i != pdata->num_leds; ++i) {
		...
		led_data = kzalloc(sizeof(struct s2mpb02_led_data), GFP_KERNEL);
...
static void s2mpb02_led_remove(struct platform_device *pdev)
{
