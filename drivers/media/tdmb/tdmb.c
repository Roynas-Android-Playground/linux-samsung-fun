 ...
 *
 * T-DMB driver for Samsung Exynos8890 (Herolte)
 *
 * Copyright (C) 2011-2024 Samsung Electronics
 * Copyright (C) 2024-2025 Herolte T-DMB port
 *
...
static void tdmb_gpio_on(void)
{
	...
	gpiod_direction_output(dt_pdata->tdmb_en_gpio, 0);
	usleep_range(1000, 1000);
	gpiod_direction_output(dt_pdata->tdmb_en_gpio, 1);
	usleep_range(25000, 25000);

	if (dt_pdata->tdmb_use_rst) {
		gpiod_direction_output(dt_pdata->tdmb_rst_gpio, 0);
		usleep_range(2000, 2000);
		gpiod_direction_output(dt_pdata->tdmb_rst_gpio, 1);
...
#ifdef CONFIG_TDMB_WAKE_LOCK_ENABLE
	pm_wakeup_event(dmb_device, 0);
#endif
	tdmb_pwr_on = true;
	return true;
...
bool tdmb_control_irq(bool set)
{
	...
	if (set) {
		irq_ret = devm_request_irq(dmb_device, dt_pdata->tdmb_irq,
					   tdmb_irq_handler,
		...
		free_irq(dt_pdata->tdmb_irq, NULL);
	}
...
static int tdmb_ant_det_check_value(void)
{
	...
	for (loop = 0; loop < TDMB_ANT_CHECK_COUNT; loop++) {
		...
		cur_val = gpiod_get_value_cansleep(dt_pdata->tdmb_ant_gpio);
...
bool tdmb_ant_det_irq_set(bool set)
{
	bool ret = true;
	int ant_irq;
	DPRINTK("%s : set(%d) ant_gpio(%p)\n", __func__, set, dt_pdata->tdmb_ant_gpio);

	if (set) {
		if (ant_irq_ret < 0) {
			ant_irq = gpiod_to_irq(dt_pdata->tdmb_ant_gpio);
			if (ant_irq < 0) {
				DPRINTK("%s: gpiod_to_irq failed\n", __func__);
				return false;
			}

			ant_prev_status =
				gpiod_get_value_cansleep(dt_pdata->tdmb_ant_gpio);

			ant_irq_ret = devm_request_threaded_irq(dmb_device, ant_irq,
								tdmb_ant_det_irq_handler, NULL,
...
static struct tdmb_dt_platform_data *get_tdmb_dt_pdata(struct device *dev)
{
	...
	pdata->tdmb_en_gpio = devm_gpiod_get(dev, "tdmb_en", GPIOD_ASIS);
	if (IS_ERR(pdata->tdmb_en_gpio)) {
		DPRINTK("Failed to get tdmb_en GPIO\n");
		goto err;
	}

	pdata->tdmb_use_rst = of_property_read_bool(dev->of_node, "tdmb_use_rst");
	if (pdata->tdmb_use_rst) {
		pdata->tdmb_rst_gpio = devm_gpiod_get(dev, "tdmb_rst", GPIOD_ASIS);
		if (IS_ERR(pdata->tdmb_rst_gpio)) {
			DPRINTK("Failed to get tdmb_rst GPIO\n");
			goto err;
		}
	} else {
	...
	if (pdata->tdmb_use_irq) {
		pdata->tdmb_irq = gpiod_to_irq(devm_gpiod_get(dev, "tdmb_irq", GPIOD_ASIS));
		if (pdata->tdmb_irq < 0) {
			DPRINTK("Failed to get tdmb_irq GPIO number\n");
			goto err;
		}
	} else {
	...
	pdata->tdmb_ant_gpio = devm_gpiod_get(dev, "tdmb_ant", GPIOD_ASIS);
	if (IS_ERR(pdata->tdmb_ant_gpio)) {
			DPRINTK("Failed to get tdmb_ant_irq GPIO\n");
			goto err;
	}
#endif
...
static int tdmb_probe(struct platform_device *pdev)
{
	...
	tdmb_class = class_create(TDMB_DEV_NAME);
...
static void tdmb_remove(struct platform_device *pdev)
{
	DPRINTK("tdmb_remove!\n");
#if defined(CONFIG_TDMB_ANT_DET)
	tdmb_ant_det_unreg_input();
