...
static inline bool ipc_active(struct mem_link_device *mld)
{
	...
	if (unlikely(!cp_online(mc))) {
		mif_err("%s<->%s: %s.state %s != ONLINE <%pf>\n", ld->name,
			mc->name, mc->name, mc_state(mc), CALLER);
		return false;
	}
	...
	if (atomic_read(&mld->forced_cp_crash)) {
		mif_err("%s<->%s: ERR! forced_cp_crash:%d <%pf>\n", ld->name,
			mc->name, atomic_read(&mld->forced_cp_crash), CALLER);
		return false;
	}
...
static void handle_no_cp_crash_ack(struct timer_list *timer)
{
	struct mem_link_device *mld =
		container_of(timer, struct mem_link_device, crash_ack_timer);
	struct link_device *ld = &mld->link_dev;
	struct modem_ctl *mc = ld->mc;
...
static void shmem_forced_cp_crash(struct mem_link_device *mld)
{
	...
	if (mld->attrs & LINK_ATTR(LINK_ATTR_MEM_DUMP)) {
		 ...
		 */
		timer_setup(&mld->crash_ack_timer, handle_no_cp_crash_ack, 0);
		mod_timer(&mld->crash_ack_timer,
			  jiffies + FORCE_CRASH_ACK_TIMEOUT);
...
static bool rild_ready(struct link_device *ld)
{
	...
	mif_err("%s: %s.opened=%d, %s.opened=%d\n", ld->name, fmt_iod->name,
		fmt_opened, rfs_iod->name, rfs_opened);
...
static struct sk_buff *rxq_read(struct mem_link_device *mld,
				struct mem_ipc_device *dev, unsigned int in)
{
	struct link_device *ld = &mld->link_dev;
	struct sk_buff *skb;
	...
	if (unlikely(!sipc5_start_valid(hdr))) {
		mif_err("%s: ERR! %s BAD CFG 0x%02X (in:%d out:%d rest:%d)\n",
			ld->name, dev->name, hdr[SIPC5_CONFIG_OFFSET], in, out,
			rest);
		goto bad_msg;
	}
...
bad_msg:
	mif_err("%s%s%s: ERR! BAD MSG: %02x %02x %02x %02x\n", ld->name,
		arrow(RX), ld->mc->name, hdr[0], hdr[1], hdr[2], hdr[3]);
...
static int rx_net_frames_from_rb(struct sbd_ring_buffer *rb, int budget)
{
	...
	if (rcvd < num_frames) {
		...
		mif_err("%s: %s<-%s: WARN! rcvd %d < num_frames %d\n", ld->name,
			iod->name, mc->name, rcvd, num_frames);
	}
	...
	while (qlen-- > 0) {
		...
		if (ret == -ENOMEM) {
			...
			if (!work_pending(&mld->page_reclaim_work)) {
				...
				queue_work(ld->rx_wq, &mld->page_reclaim_work);
			}
			return;
		}
		msb_free(msb);
	...
	case IPC_RAW:
		if (unlikely(atomic_read(&ld->netif_stopped) > 0)) {
			if (skb->queue_mapping != 1) {
				...
				mif_err_limited(
					"Tx_flowctrl, but received ack from ch %d\n",
					ch);
			}
		}
...
static void shmem_boot_on(struct link_device *ld, struct io_device *iod)
{
	...
	if (ld->sbd_ipc) {
		...
		if (mld->iosm) {
			...
			mif_info(
				"Control message region has been initialized\n");
		}
	}
				  ...
				  unsigned long arg)
{
	...
	arm_smccc_smc(SMC_ID, msr.mode, param2, param3, 0, 0, 0, 0, &res);
	/* exynos_smc(SMC_ID_CLK, SSS_CLK_DISABLE, 0, 0); */
	mif_info("%s: return_value=%ld\n", ld->name, res.a0);
	return res.a0;
}
			   ...
			   unsigned long arg)
{
	...
	while (copied < mld->size) {
		...
		memcpy(skb_put(skb, alloc_size), mld->base + copied,
		       alloc_size);
		copied += alloc_size;
...
static void shmem_cp2ap_pcie_l1ss_disable_handler(void *data)
{
	...
	// TODO: Linkage with exynos_pcie_l1ss_ctrl() is needed for modem interface.
	if (req == 1) {
...
{
	ld->rx_wq = alloc_workqueue("mem_rx_work",
...
static void shmem_cp2ap_wakelock_handler(void *data)
{
	...
	if (req == 0) {
		__pm_relax(mld->cp_wakelock);
		mif_err("cp_wakelock unlocked\n");
	} else if (req == 1) {
		__pm_stay_awake(mld->cp_wakelock);
...
struct link_device *shmem_create_link_device(struct platform_device *pdev)
{
	...
	bool msg_irq_registered = false;
	bool wakelock_irq_registered = false;
	bool pcie_irq_registered = false;
	bool status_irq_registered = false;
	int err;
	mif_err("+++\n");
	...
	*/
	...
	if (ld->sbd_ipc) {
		...
		err = create_sbd_link_device(ld, &mld->sbd_link_dev, mld->base,
					     mld->size);
		if (err < 0)
			goto error;
	}
	...
	if (err) {
		mif_err("%s: ERR! mbox_request_irq(%u) fail (%d)\n", ld->name,
			mld->irq_cp2ap_msg, err);
		goto error;
	}
	msg_irq_registered = true;
	...
	mld->cp_wakelock = wakeup_source_register(&pdev->dev, "cp_wakelock");
	...
	wakelock_irq_registered = true;

	mld->mbx_cp2ap_pcie_l1ss_disable =
		modem->mbx->mbx_cp2ap_pcie_l1ss_disable;
	mld->irq_cp2ap_pcie_l1ss_disable =
		modem->mbx->irq_cp2ap_pcie_l1ss_disable;

	err = mbox_request_irq(mld->irq_cp2ap_pcie_l1ss_disable,
			       shmem_cp2ap_pcie_l1ss_disable_handler, mld);
	if (err) {
		mif_err("%s: ERR! mbox_request_irq(%u) fail (%d)\n", ld->name,
			mld->irq_cp2ap_pcie_l1ss_disable, err);
		goto error;
	}
	pcie_irq_registered = true;
	...
	err = mbox_request_irq(mld->irq_cp2ap_status, shmem_tx_state_handler,
			       mld);
	if (err) {
		mif_err("%s: ERR! mbox_request_irq(%u) fail (%d)\n", ld->name,
			mld->irq_cp2ap_status, err);
		goto error;
	}
	status_irq_registered = true;
...
error:
	if (status_irq_registered)
		mcu_ipc_unregister_handler(mld->irq_cp2ap_status,
					   shmem_tx_state_handler);
	if (pcie_irq_registered)
		mcu_ipc_unregister_handler(mld->irq_cp2ap_pcie_l1ss_disable,
					   shmem_cp2ap_pcie_l1ss_disable_handler);
	if (wakelock_irq_registered)
		mcu_ipc_unregister_handler(mld->irq_cp2ap_wakelock,
					   shmem_cp2ap_wakelock_handler);
	if (msg_irq_registered)
		mcu_ipc_unregister_handler(mld->irq_cp2ap_msg,
					   shmem_irq_handler);
	if (mld->cp_wakelock)
		wakeup_source_unregister(mld->cp_wakelock);
	if (ld && ld->rx_wq)
