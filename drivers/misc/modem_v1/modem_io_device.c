...
static ssize_t show_waketime(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	unsigned int msec;
	char *p = buf;
	...
	struct io_device *iod =
		container_of(miscdev, struct io_device, miscdev);
...
static ssize_t store_waketime(struct device *dev, struct device_attribute *attr,
...
static ssize_t store_loopback(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct miscdevice *miscdev = dev_get_drvdata(dev);
	struct modem_shared *msd =
...
static ssize_t show_txlink(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
	struct miscdevice *miscdev = dev_get_drvdata(dev);
	struct modem_shared *msd =
...
static ssize_t store_txlink(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
...
{
	__pm_wakeup_event(iod->wakelock,
...
static long misc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	unsigned long size;
	int tx_link;
	const char *registered_name = READ_ONCE(iod->miscdev.name);

	if (unlikely(READ_ONCE(iod->name) != registered_name)) {
		pr_err("mif: %s: io_device %px name pointer changed from %px to %px before ioctl %#x; refusing operation\n",
		       __func__, iod, registered_name, READ_ONCE(iod->name), cmd);
		return -EUCLEAN;
	}
	...
	case IOCTL_MODEM_RESET:
		if (mc->ops.modem_reset) {
			...
			if (unlikely(READ_ONCE(iod->name) != registered_name)) {
				pr_err("mif: %s: io_device %px name changed from %px to %px during modem_reset; refusing further use\n",
				       __func__, iod, registered_name,
				       READ_ONCE(iod->name));
				return -EUCLEAN;
			}
			return ret;
	...
	case IOCTL_SECURITY_REQ:
		if (ld->security_req) {
			...
			if (unlikely(READ_ONCE(iod->name) != registered_name)) {
				pr_err("mif: %s: io_device %px name changed from %px to %px during security_req; refusing further use\n",
				       __func__, iod, registered_name,
				       READ_ONCE(iod->name));
				return -EUCLEAN;
			}
			  ...
			  size_t count, loff_t *fpos)
{
	...
	while (copied < cnt) {
		...
		if (cfg) {
			...
			sipc5_build_header(iod, buff, cfg, tx_bytes,
					   cnt - copied);
		}
		...
		if (ret < 0) {
			mif_err("%s->%s: %s->send fail(%d, tx:%d len:%lu)\n",
				iod->name, mc->name, ld->name, ret, tx_bytes,
				(unsigned long)count);
			dev_kfree_skb_any(skb);
			return ret;
		}
	...
	if (copied_frm != tot_frame) {
		mif_info("%s->%s: WARN! %s->send ret:%d (len:%lu)\n", iod->name,
			 mc->name, ld->name, copied_frm, (unsigned long)count);
	}
...
{
	return (skb && skb->priority == RAW_HPRIO) ? 1 : 0;
}
...
int sipc5_init_io_device(struct io_device *iod)
{
	...
	case IODEV_NET:
		...
		if (iod->use_handover)
			iod->ndev = alloc_netdev_mqs(
				0, iod->name, NET_NAME_UNKNOWN,
				vnet_setup_ether, MAX_NDEV_TX_Q, MAX_NDEV_RX_Q);
		else
			iod->ndev = alloc_netdev_mqs(0, iod->name,
						     NET_NAME_UNKNOWN,
						     vnet_setup, MAX_NDEV_TX_Q,
						     MAX_NDEV_RX_Q);
...
void sipc5_deinit_io_device(struct io_device *iod)
{
	...
	wakeup_source_unregister(iod->wakelock);
