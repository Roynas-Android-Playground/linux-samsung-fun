...
static struct modem_shared *
create_modem_shared_data(struct platform_device *pdev)
{
	...
	msd->storage.cnt = 0;
	msd->storage.addr = devm_kzalloc(
		dev, MAX_MIF_BUFF_SIZE + (MAX_MIF_SEPA_SIZE * 2), GFP_KERNEL);
	if (!msd->storage.addr) {
			  ...
			  struct device *dev)
{
	...
	iod->wakelock = wakeup_source_register(dev, iod->name);
					   ...
					   struct modem_data *pdata)
{
	...
	pdata->gpio_ipc_int2cp =
		of_get_named_gpio(np, "mif,gpio_ipc_int2cp", 0);
...
static int __parse_dt_optional_gpio_pdata(struct device *dev,
					  struct modem_data *pdata)
{
#if defined(CONFIG_LINK_POWER_MANAGEMENT) && \
	!defined(CONFIG_LINK_POWER_MANAGEMENT_WITH_FSM)

	/* GPIO_CP_WAKEUP */
	pdata->gpio_cp_wakeup =
		devm_gpiod_get_optional(dev, "cp-wakeup", GPIOD_OUT_LOW);
	if (IS_ERR(pdata->gpio_cp_wakeup))
		return PTR_ERR(pdata->gpio_cp_wakeup);

	/* GPIO_AP_STATUS */
	pdata->gpio_ap_status =
		devm_gpiod_get_optional(dev, "ap-status", GPIOD_OUT_LOW);
	if (IS_ERR(pdata->gpio_ap_status))
		return PTR_ERR(pdata->gpio_ap_status);

	/* GPIO_AP_WAKEUP */
	pdata->gpio_ap_wakeup =
		devm_gpiod_get_optional(dev, "ap-wakeup", GPIOD_IN);
	if (IS_ERR(pdata->gpio_ap_wakeup))
		return PTR_ERR(pdata->gpio_ap_wakeup);

	/* GPIO_CP_STATUS */
	pdata->gpio_cp_status =
		devm_gpiod_get_optional(dev, "cp-status", GPIOD_IN);
	if (IS_ERR(pdata->gpio_cp_status))
		return PTR_ERR(pdata->gpio_cp_status);
#endif
	return 0;
}

static int parse_dt_gpio_pdata(struct device *dev, struct modem_data *pdata)
{
	int err = 0;
...
#ifndef CONFIG_LINK_DEVICE_SHMEM
	err = __parse_dt_mandatory_gpio_pdata(dev->of_node, pdata);
	if (err)
		return err;
#endif
	err = __parse_dt_optional_gpio_pdata(dev, pdata);
	if (err)
		return err;

#if defined(CONFIG_LINK_POWER_MANAGEMENT_WITH_FSM)
	pm_node = of_get_child_by_name(dev->of_node, PM_DT_NODE_NAME);
	if (!pm_node) {
...
#define READ_INDEX(_property, _field, _limit) do { \
	ret = of_property_read_u32(np, (_property), &(_field)); \
	if (ret) \
		return ret; \
	if ((_field) >= (_limit)) \
		return -ERANGE; \
	...
	READ_INDEX("mif,int_ap2cp_msg", mbox->int_ap2cp_msg, 16);
	READ_INDEX("mif,int_ap2cp_wakeup", mbox->int_ap2cp_wakeup, 16);
	READ_INDEX("mif,int_ap2cp_status", mbox->int_ap2cp_status, 16);
	READ_INDEX("mif,int_ap2cp_active", mbox->int_ap2cp_active, 16);
	READ_INDEX("mif,irq_cp2ap_msg", mbox->irq_cp2ap_msg, 16);
	READ_INDEX("mif,irq_cp2ap_wakeup", mbox->irq_cp2ap_wakeup, 16);
	READ_INDEX("mif,irq_cp2ap_status", mbox->irq_cp2ap_status, 16);
	READ_INDEX("mif,irq_cp2ap_perf_req", mbox->irq_cp2ap_perf_req, 16);
	READ_INDEX("mif,irq_cp2ap_perf_req_cpu", mbox->irq_cp2ap_perf_req_cpu, 16);
	READ_INDEX("mif,irq_cp2ap_perf_req_mif", mbox->irq_cp2ap_perf_req_mif, 16);
	READ_INDEX("mif,irq_cp2ap_perf_req_int", mbox->irq_cp2ap_perf_req_int, 16);
	READ_INDEX("mif,irq_cp2ap_active", mbox->irq_cp2ap_active, 16);
	READ_INDEX("mif,irq_cp2ap_wakelock", mbox->irq_cp2ap_wakelock, 16);
	READ_INDEX("mif,irq_cp2ap_pcie_l1ss_disable",
		   mbox->irq_cp2ap_pcie_l1ss_disable, 16);

	READ_INDEX("mbx_ap2cp_msg", mbox->mbx_ap2cp_msg, MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_msg", mbox->mbx_cp2ap_msg, MAX_MBOX_NUM);
	READ_INDEX("mbx_ap2cp_wakeup", mbox->mbx_ap2cp_wakeup, MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_wakeup", mbox->mbx_cp2ap_wakeup, MAX_MBOX_NUM);
	READ_INDEX("mbx_ap2cp_status", mbox->mbx_ap2cp_status, MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_status", mbox->mbx_cp2ap_status, MAX_MBOX_NUM);
	READ_INDEX("mbx_ap2cp_active", mbox->mbx_ap2cp_active, MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_dvfsreq", mbox->mbx_cp2ap_perf_req, MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_dvfsreq_cpu", mbox->mbx_cp2ap_perf_req_cpu,
		   MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_dvfsreq_mif", mbox->mbx_cp2ap_perf_req_mif,
		   MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_dvfsreq_int", mbox->mbx_cp2ap_perf_req_int,
		   MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_active", mbox->mbx_cp2ap_active, MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_wakelock", mbox->mbx_cp2ap_wakelock,
		   MAX_MBOX_NUM);
	READ_INDEX("mbx_cp2ap_pcie_l1ss_disable",
		   mbox->mbx_cp2ap_pcie_l1ss_disable, MAX_MBOX_NUM);
	READ_INDEX("mbx_ap2cp_sec", mbox->mbx_ap2cp_sec, MAX_MBOX_NUM);
	READ_INDEX("mbx_ap2cp_usec", mbox->mbx_ap2cp_usec, MAX_MBOX_NUM);
	READ_INDEX("mbx_ap2cp_et_dac_cal", mbox->mbx_ap2cp_et_dac_cal,
		   MAX_MBOX_NUM);
	READ_INDEX("mbx_ap2cp_lock_value", mbox->mbx_ap2cp_lock_value,
		   MAX_MBOX_NUM);
	...
	if (!prop || !prop->value) {
		...
		mbox->ap_clk_table =
			devm_kzalloc(dev, sizeof(u32) * nr, GFP_KERNEL);
...
static struct modem_data *modem_if_parse_dt_pdata(struct device *dev)
{
	...
	if (parse_dt_gpio_pdata(dev, pdata)) {
...
{
	return single_open(file, simslot_count, inode);
}
...
static int modem_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(modemctl)) {
		int err = PTR_ERR(modemctl);

		mif_err("%s: modemctl failed: %d\n", pdata->name, err);
		devm_kfree(dev, msd);
		return err;
	}
	...
	for (i = 0; i < pdata->num_iodevs; i++) {
		...
		attach_devices(iod[i], pdata->iodevs[i].tx_link, &pdev->dev);
	}
