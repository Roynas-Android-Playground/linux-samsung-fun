...
#define pktlog_tx_bottom_skb(mld, skb) \
	pktlog_queue_skb((mld)->pktlog, (PKT_TX | PKT_BOT), skb)
#define pktlog_tx_top_skb(mld, skb) \
	pktlog_queue_skb((mld)->pktlog, (PKT_TX | PKT_TOP), skb)
#define pktlog_rx_bottom_skb(mld, skb) \
	pktlog_queue_skb((mld)->pktlog, (PKT_RX | PKT_BOT), skb)
#define pktlog_rx_top_skb(mld, skb) \
	pktlog_queue_skb((mld)->pktlog, (PKT_RX | PKT_TOP), skb)
#define pktlog_text_buf(mld, buf) \
	do {                      \
	} while (0)

#else /* else of DEBUG_PKTLOG */
static inline struct pktlog_data *create_pktlog(char *name)
{
	return NULL;
}
#define remove_pktlog(pkt) \
	do {               \
	} while (0)
#define pktlog_tx_bottom_skb(mld, skb) \
	do {                           \
