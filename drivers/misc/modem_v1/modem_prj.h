...
enum link_state { LINK_STATE_OFFLINE = 0, LINK_STATE_IPC, LINK_STATE_CP_CRASH };
...
{
	return (ch >= SIPC_CH_ID_PDP_0 && ch <= SIPC_CH_ID_PDP_14) ? true :
								     false;
}
...
rx_alloc_skb(unsigned int length, struct io_device *iod, struct link_device *ld)
{
	struct sk_buff *skb;
...
struct modem_ctl {
	...
	char *name;
	struct modem_data *mdm_data;
	struct gpio_descs *hw_rev_gpios;
