...
enum ipc_layer { LINK, IODEV, APP, MAX_SIPC_LAYER };

static const char const *sipc_layer_string[] = { [LINK] = "LNK",
						 [IODEV] = "IOD",
						 [APP] = "APP",
...
static const char const *dev_format_string[] = {
	[IPC_FMT] = "FMT",   [IPC_RAW] = "RAW",
	[IPC_RFS] = "RFS",   [IPC_MULTI_RAW] = "MULTI_RAW",
	[IPC_BOOT] = "BOOT", [IPC_DUMP] = "DUMP",
	[IPC_CMD] = "CMD",   [IPC_DEBUG] = "DEBUG",
};
...
static const char const *direction_string[] = { [TX] = "TX", [RX] = "RX" };
...
static const char const *udl_string[] = { [UL] = "UL", [DL] = "DL" };
...
static const char const *q_direction_string[] = { [TX] = "TXQ", [RX] = "RXQ" };
...
static const char const *ipc_direction_string[] = { [TX] = "AP->CP",
						    [RX] = "AP<-CP" };
...
static const char const *arrow_direction[] = { [TX] = "->", [RX] = "<-" };
...
struct __packed utc_time {
	u32 year : 18, mon : 4, day : 5, hour : 5;
	u32 min : 6, sec : 6, us : 20;
};
...
void mif_ipc_log(enum mif_log_id, struct modem_shared *, const char *, size_t);
void _mif_irq_log(enum mif_log_id, struct modem_shared *, struct mif_irq_map,
		  const char *, size_t);
void _mif_com_log(enum mif_log_id, struct modem_shared *, const char *, ...);
void _mif_time_log(enum mif_log_id, struct modem_shared *, struct timespec64,
		   const char *, size_t);
...
static inline struct io_device *link_get_iod_with_format(struct link_device *ld,
							 enum dev_format format)
{
...
static inline struct io_device *get_iod_with_channel(struct modem_shared *msd,
						     unsigned int channel)
{
	return msd->ch2iod[channel];
}

static inline struct io_device *
link_get_iod_with_channel(struct link_device *ld, unsigned int channel)
{
