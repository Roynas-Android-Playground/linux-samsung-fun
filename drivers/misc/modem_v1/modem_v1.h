...
enum read_write { RD = 0, WR = 1, RDWR = 2 };
...
#ifdef CONFIG_LINK_DEVICE_SHMEM
enum shmem_type { REAL_SHMEM, C2C_SHMEM, MAX_SHMEM_TYPE };
...
struct modem_data {
	...
	struct gpio_desc *gpio_ap_wakeup;
	unsigned int irq_ap_wakeup;
	struct gpio_desc *gpio_ap_status;
	struct gpio_desc *gpio_cp_wakeup;
	struct gpio_desc *gpio_cp_status;
...
#define mif_dt_read_string(np, prop, dest)                                   \
	do {                                                                 \
		if (of_property_read_string(np, prop, (const char **)&dest)) \
			return -EINVAL;                                      \
	} while (0)
...
#define mif_err_limited(fmt, ...)                                         \
	printk_ratelimited(KERN_ERR LOG_TAG "%s: " pr_fmt(fmt), __func__, \
			   ##__VA_ARGS__)
#define mif_err(fmt, ...) \
...
#define mif_trace(fmt, ...)                                           \
	printk(KERN_DEBUG "mif: %s: %d: called(%pF): " fmt, __func__, \
