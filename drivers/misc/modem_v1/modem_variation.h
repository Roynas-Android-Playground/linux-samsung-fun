...
#define DECLARE_MODEM_INIT(type)                                     \
	static int type##_init_modemctl_device(struct modem_ctl *mc, \
					       struct modem_data *pdata)

#define DECLARE_MODEM_INIT_DUMMY(type) \
	DECLARE_MODEM_INIT(type)       \
	{                              \
		return 0;              \
	}

#define DECLARE_LINK_INIT(type)                               \
	static struct link_device *type##_create_link_device( \
		struct platform_device *pdev)

#define DECLARE_LINK_INIT_DUMMY(type) \
	DECLARE_LINK_INIT(type)       \
	{                             \
		return NULL;          \
	}
