...
static u32 exynos_smc_read(enum cp_control reg)
{
	...
	arm_smccc_smc(SMC_ID, READ_CTRL, 0, reg, 0, 0, 0, 0, &res);
	/* exynos_smc() returned x0; arm_smccc exposes the same value in a0. */
	cp_ctrl = res.a0;
	if (!(cp_ctrl & 0xffff)) {
		cp_ctrl >>= 16;
	} else {
...
static u32 exynos_smc_write(enum cp_control reg, u32 value)
{
	...
	arm_smccc_smc(SMC_ID, WRITE_CTRL, value, reg, 0, 0, 0, 0, &res);
	ret = (s32)res.a0;
	if (ret > 0) {
...
#else
	ret = regmap_update_bits(mc->pmureg, EXYNOS_PMU_CP_CTRL_S, CP_START,
				 CP_START);
	if (ret < 0)
		pr_err("ERR! CP Release Fail: %d\n", ret);
	else {
...
int exynos_cp_init(struct modem_ctl *mc)
{
	...
	ret = regmap_update_bits(mc->pmureg, EXYNOS_PMU_CP_CTRL_NS, CP_PWRON,
				 0);
	if (ret < 0)
		pr_err("%s: ERR! CP_PWRON Fail: %d\n", __func__, ret);

	ret = regmap_update_bits(mc->pmureg, EXYNOS_PMU_CP_CTRL_S, CP_START, 0);
	if (ret < 0)
	...
	if (mode == CP_POWER_ON) {
		...
		if (ret < 0)
			...
			pr_info("%s: CP Start: [0x%08X] -> [0x%08X]\n",
				__func__, cp_ctrl, exynos_smc_read(CP_CTRL_S));
	} else {
		ret = exynos_smc_write(CP_CTRL_NS, cp_ctrl & ~CP_PWRON);
		if (ret < 0)
