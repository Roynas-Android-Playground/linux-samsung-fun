 ...
 * dedicated driver. This driver supports the UTMI/USB2 path only;
 * the PIPE3/SuperSpeed PHY is not registered.
 */
...
static int exynos8890_usbdrd_clks_enable(struct exynos8890_usbdrd_phy *phy)
{
	int ret;
	...
	if (!ret)
		phy->clks_enabled = true;
	return ret;
}

static void exynos8890_usbdrd_clks_disable(struct exynos8890_usbdrd_phy *phy)
{
	if (!phy->clks_enabled)
		return;
	clk_bulk_disable_unprepare(ARRAY_SIZE(phy->clks), phy->clks);
	phy->clks_enabled = false;
}
				      ...
				      bool host)
{
	...
	/* Host and device tuning for the Exynos8890 USB3DRD HS block. */
...
static void exynos8890_usbdrd_qchannel_enable(struct exynos8890_usbdrd_phy *phy)
{
	...
	/* Disable automatic Q-active inputs before forcing the channel active. */
...
static int exynos8890_usbdrd_hw_init(struct exynos8890_usbdrd_phy *phy)
{
	...
	/* Configure the v1.01 PHY for the internal 26 MHz reference. */
	phyclkrst = readl(phy->regs + EXYNOS8890_DRD_PHYCLKRST);
	phyclkrst |= PHYCLKRST_PORTRESET;
	phyclkrst &= ~PHYCLKRST_REFCLKSEL;
	/*
	 * The v1.01 PHY encodes DIFF_INTERNAL as 0x4 << 2. Bit 4
...
static int exynos8890_usbdrd_init(struct phy *generic_phy)
{
	...
	ret = exynos8890_usbdrd_clks_enable(phy);
	if (ret)
		return ret;

	ret = exynos8890_usbdrd_hw_init(phy);
	if (ret)
		exynos8890_usbdrd_clks_disable(phy);
...
{
	struct exynos8890_usbdrd_phy *phy = phy_get_drvdata(generic_phy);
	bool was_enabled = phy->clks_enabled;
	int ret;
