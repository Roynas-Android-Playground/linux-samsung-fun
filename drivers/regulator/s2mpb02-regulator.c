...
static int s2m_set_voltage_sel_regmap(struct regulator_dev *rdev, unsigned sel)
{
	...
	ret = s2mpb02_update_reg(i2c, rdev->desc->vsel_reg, sel,
				 rdev->desc->vsel_mask);
	if (ret < 0)
		goto out;
				       ...
				       struct s2mpb02_platform_data *pdata)
{
	...
	rdata = devm_kcalloc(iodev->dev, ARRAY_SIZE(regulators),
			     sizeof(*rdata),
			     GFP_KERNEL);
	if (!rdata) {
		dev_err(iodev->dev,
	...
	for_each_child_of_node(regulators_np, reg_np) {
		...
		rdata = &pdata->regulators[pdata->num_regulators++];
		rdata->id = i;
		rdata->initdata =
			of_get_regulator_init_data(iodev->dev, reg_np, NULL);
...
static int s2mpb02_pmic_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_regulators; i++) {
		...
		s2mpb02->rdev[i] = regulator_register(&pdev->dev,
...
static void s2mpb02_pmic_remove(struct platform_device *pdev)
{
