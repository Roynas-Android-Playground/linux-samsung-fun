 ...
 *
 * Regmap-based CPU and cluster power controls:
 *
...
/*
 * Callers must enforce the boot-cluster power-down restriction; this
 * low-level interface does not select cluster policy.
 */
int exynos8890_cluster_up(unsigned int cluster)
{
	...
	if (!ret)
		ret = regmap_read(pmureg, PMU_CPUSEQ_OPTION_BASE +
...
static int exynos8890_cpupm_probe(struct platform_device *pdev)
{
	...
	for (cluster = 0; cluster < 2; cluster++) {
		ret = regmap_read(regmap, PMU_CPUSEQ_OPTION_BASE +
				  cluster * PMU_CLUSTER_ADDR_OFFSET, &val);
		if (ret)
			return dev_err_probe(&pdev->dev, ret,
					     "failed to read cluster%u sequencer\n",
					     cluster);
	...
	for_each_possible_cpu(cpu) {
		ret = regmap_read(regmap, PMU_CPU_CONFIG_BASE + pmu_cpu_offset(cpu),
				  &val);
		if (ret)
			return dev_err_probe(&pdev->dev, ret,
					     "failed to read CPU%u power config\n", cpu);
		pr_info("exynos8890-cpupm: cpu%u CPU_CONFIG=%#x local_pwr_cfg=%#x\n",
