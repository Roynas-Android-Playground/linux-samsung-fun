...
static void asv_set_freq_limit(void)
{
	...
	asv_block = exynos8890_ect_get_block("ASV");
	if (asv_block == NULL)
		BUG();
...
retry:
	...
	domain = exynos8890_ect_asv_get_domain(asv_block, "dvfs_big");
	if (!domain)
		BUG();
	...
	domain = exynos8890_ect_asv_get_domain(asv_block, "dvfs_little");
	if (!domain)
		BUG();
	get_max_min_freq_lv(domain, asv_tbl_info.asv_table_ver, &max_lv, &min_lv);
	if (max_lv >= 0)
	...
	domain = exynos8890_ect_asv_get_domain(asv_block, "dvfs_g3d");
	if (!domain)
		BUG();
	get_max_min_freq_lv(domain, asv_tbl_info.asv_table_ver, &max_lv, &min_lv);
	if (max_lv >= 0)
	...
	domain = exynos8890_ect_asv_get_domain(asv_block, "dvfs_mif");
	if (!domain)
		BUG();
	get_max_min_freq_lv(domain, asv_tbl_info.asv_table_ver, &max_lv, &min_lv);
	if (max_lv >= 0)
	...
	domain = exynos8890_ect_asv_get_domain(asv_block, "dvfs_int");
	if (!domain)
		BUG();
	get_max_min_freq_lv(domain, asv_tbl_info.asv_table_ver, &max_lv, &min_lv);
	if (max_lv >= 0)
	...
	domain = exynos8890_ect_asv_get_domain(asv_block, "dvfs_disp");
	if (!domain)
		BUG();
	get_max_min_freq_lv(domain, asv_tbl_info.asv_table_ver, &max_lv, &min_lv);
	if (max_lv >= 0)
	...
	domain = exynos8890_ect_asv_get_domain(asv_block, "dvfs_cam");
	if (!domain)
		BUG();
	get_max_min_freq_lv(domain, asv_tbl_info.asv_table_ver, &max_lv, &min_lv);
	if (max_lv >= 0)
...
/*
 * Real definition for smc.h's "extern int exynos_smc_readsfr(...)" -
 * same arm_smccc_smc() pattern as asv_get_ids_info() below and
 * exynos8890-cpctl.c:exynos8890_cpctl_smc(). This used to be a local
 * static inline hand-rolled with raw register-variable inline asm
 * (duplicating SMC_CMD_REG/SMC_REG_CLASS_SFR_R, now real constants
 * from smc.h); replaced once pwrcal-env.h started pulling in the real
 * <linux/smc.h> declaration for this symbol.
 */
int exynos_smc_readsfr(unsigned long addr, unsigned long *val)
{
	...
	arm_smccc_smc(SMC_CMD_REG, SMC_REG_ID_SFR_R(addr), 0, 0, 0, 0, 0, 0, &result);
...
static void asv_voltage_init_table(struct asv_table_list **asv_table, struct pwrcal_vclk_dfs *dfs)
{
	...
	margin_block = exynos8890_ect_get_block("MARGIN");

	domain = exynos8890_ect_asv_get_domain(asv_block, dfs->vclk.name);
	if (domain == NULL)
		return;
	...
	tim_block = exynos8890_ect_get_block(BLOCK_TIMING_PARAM);
	if ((strcmp("dvfs_mif", dfs->vclk.name) == 0) && (tim_block != 0)) {
		ect_query_key = (pwrcal_get_dram_manufacturer() & 0xffffffffffffff00) | 0x3;
		ect_mif = exynos8890_ect_timing_param_get_key(tim_block, ect_query_key);
		if (ect_mif)
			mif_volt_margin = ect_mif->timing_parameter;
	}

	if (margin_block)
		margin_domain = exynos8890_ect_margin_get_domain(margin_block, dfs->vclk.name);
...
static void asv_ssa_init(void)
{
	...
	gen_block = exynos8890_ect_get_block("GEN");
	if (gen_block == NULL)
		return;

	param = exynos8890_ect_gen_param_get_table(gen_block, "SSA_BIG");
	if (param) {
	...
	param = exynos8890_ect_gen_param_get_table(gen_block, "SSA_LITTLE");
	if (param) {
		asv_table_version = asv_tbl_info.asv_table_ver;
		if (asv_table_version >= param->num_of_row)
			asv_table_version = param->num_of_row - 1;
	...
	param = exynos8890_ect_gen_param_get_table(gen_block, "SSA_G3D");
	if (param) {
		asv_table_version = asv_tbl_info.asv_table_ver;
		if (asv_table_version >= param->num_of_row)
			asv_table_version = param->num_of_row - 1;
	...
	param = exynos8890_ect_gen_param_get_table(gen_block, "SSA_MIF");
	if (param) {
		asv_table_version = asv_tbl_info.asv_table_ver;
		if (asv_table_version >= param->num_of_row)
			asv_table_version = param->num_of_row - 1;
	...
	param = exynos8890_ect_gen_param_get_table(gen_block, "SSA_INT");
	if (param) {
		asv_table_version = asv_tbl_info.asv_table_ver;
		if (asv_table_version >= param->num_of_row)
			asv_table_version = param->num_of_row - 1;
	...
	param = exynos8890_ect_gen_param_get_table(gen_block, "SSA_CAM");
	if (param) {
		asv_table_version = asv_tbl_info.asv_table_ver;
		if (asv_table_version >= param->num_of_row)
			asv_table_version = param->num_of_row - 1;
	...
	param = exynos8890_ect_gen_param_get_table(gen_block, "SSA_DISP");
	if (param) {
		asv_table_version = asv_tbl_info.asv_table_ver;
		if (asv_table_version >= param->num_of_row)
			asv_table_version = param->num_of_row - 1;
...
static int asv_get_ids_info(unsigned int domain)
{
	...
	asm volatile("dsb sy" ::: "memory");
	arm_smccc_smc(0x82001014, 0, 0x2002, 0x18, 0, 0, 0, 0, &result);
	...
	if (domain == 0)	/* MNGS */
		ret = (int)(result.a2 & 0xFF);
	else			/* G3D */
		ret = (int)((result.a2 >> 8) & 0xFF);
...
int asv_get_information(enum dvfs_id id, enum asv_group grp, unsigned int lv) {
	...
	switch (id) {
		...
		case cal_asv_dvs_g3dm:
			max_lv = asv_dvs_g3dm->table->num_of_lv;
			domain = exynos8890_ect_asv_get_domain(asv_block, "dvs_g3dm");
