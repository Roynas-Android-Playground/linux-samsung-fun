...
void dfs_set_clk_information(struct pwrcal_vclk_dfs *dfs)
{
	...
	dvfs_block = exynos8890_ect_get_block("DVFS");
	if (dvfs_block == NULL)
		return;

	dvfs_domain = exynos8890_ect_dvfs_get_domain(dvfs_block, dfs->vclk.name);
	if (dvfs_domain == NULL)
		return;
...
void dfs_set_pscdc_information(void)
{
	...
	pscdc = exynos8890_ect_gen_param_get_table(gen_block, "PSCDC");
	if (pscdc == NULL)
		return;
