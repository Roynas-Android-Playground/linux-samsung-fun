...
void dfs_dram_param_init(void)
{
	...
	dram_block = exynos8890_ect_get_block(BLOCK_TIMING_PARAM);
	if (dram_block == NULL)
		return;
	...
	if (query_key != 0) {
		size = exynos8890_ect_timing_param_get_key(dram_block, query_key | 0x1);
	} else {
...
void dfs_mif_level_init(void)
{
	...
	domain = exynos8890_ect_dvfs_get_domain(dvfs_block, vclk_dvfs_mif.vclk.name);
	if (domain == NULL)
		return;
