...
/*
 * exynos-ss (Exynos Snapshot crash-history ring buffer) isn't ported into
 * this kernel - it's a whole separate Samsung debug subsystem, not part of
 * the decon/DSIM/VPP driver itself. decon_err()/decon_warn() already log
 * through pr_err()/pr_warn() right before this, so dropping the extra
 * crash-history copy just loses post-mortem-only debug info, not any
 * runtime behavior.
 */
#ifndef exynos_ss_printk
#define exynos_ss_printk(fmt, ...) do { } while (0)
#endif
...
struct decon_dma_buf_data {
	...
	dma_addr_t			dma_addr;
	/* acquire fence: producer's signal that the buffer is ready */
...
struct decon_reg_data {
	...
	/* retire fence: signaled once this frame's buffers are retired */
...
struct decon_device {
	...
	/*
	 * Retire-fence timeline. mainline dropped the in-kernel sw_sync
	 * timeline API (drivers/staging/android/sync.h); dma_fence's own
	 * fence_context+seqno pairing is the direct replacement - each
	 * dma_fence created here just needs a context id shared across the
	 * device and a strictly increasing seqno, same as sw_sync_pt did.
	 */
	u64				fence_context;
