...
/*
 * Raises the MIF (memory bus) frequency floor while this VPP window is
 * rotating - rotation needs much more memory bandwidth than a straight
 * blit and can underrun otherwise. This used the global PM_QOS_BUS_
 * THROUGHPUT class, which mainline dropped along with the rest of the
 * Exynos-specific pm_qos classes (only dev_pm_qos_* per-device requests
 * remain). The correct replacement is a dev_pm_qos_add_request() against
 * the MIF devfreq device, the same pattern exynos8890-cpufreq.c already
 * uses via its "samsung,mif-devfreq" DT phandle - not done here since it
 * needs a new DT binding on the VPP node. Until then this is a no-op:
 * VPP rotation gets no MIF floor boost.
 */
...
/*
 * decon->{mif,disp,int}_qos used the same now-gone global PM_QOS_BUS_
 * THROUGHPUT/PM_QOS_DISPLAY_THROUGHPUT/PM_QOS_DEVICE_THROUGHPUT classes
 * as bts_mif_lock() above - a real bandwidth-floor mechanism (raise MIF/
 * DISP/INT clocks while DECON is actively driving the panel, drop them
 * on release), not dead code. Needs the same dev_pm_qos_add_request()-
 * against-devfreq-device treatment via new DT phandles; left as no-ops
