...
#include "decon8890-of-gpio-compat.h"
...
/*
 * Real definition for smc.h's "extern int exynos_smc(...)" - same
 * arm_smccc_smc()->res.a0 pattern already used by
 * exynos8890-cpctl.c:exynos8890_cpctl_smc() and drivers/char/hw_random/
 * exyswd-rng.c's local exynos_smc() macro. This is the DRM/CFW
 * secure-buffer-protection SMC path (win_config->protection, HDCP-style
 * content protection) - only exercised once a real DRM-protected-content
 * userspace path is wired up.
 */
int exynos_smc(unsigned long cmd, unsigned long arg1, unsigned long arg2,
		unsigned long arg3)
{
...
static int cfw_buf_list_create(struct sg_table *sgt,
		struct decon_reg_data *regs, u32 id)
{
	...
	/*
	 * ion_phys() used to hand back the (physically contiguous) buffer's
	 * phys addr + length directly from the ion_handle. Now that import
	 * goes through dma_buf_map_attachment(), the same info is just the
	 * first (and, for these linear framebuffer allocations, only) sg
	 * segment of the already-mapped sg_table.
	 */
	sbuf->addr = sg_phys(sgt->sgl);
	sbuf->len = sgt->sgl->length;
		...
		struct decon_dma_buf_data *dma)
{
	...
	/*
	 * No ion_iovmm_unmap() call: this device's IOMMU domain is attached
	 * automatically by the DMA-IOMMU glue (DT iommus=) on probe, so
	 * dma_buf_unmap_attachment() below already tears down the IOMMU
	 * mapping as part of the normal DMA-API unmap.
	 */
		...
		struct dma_buf *buf, int win_no)
{
	...
	/*
	 * dma_buf_map_attachment() already runs dma_map_sg() against this
	 * device, which - since exynos-iommu.c auto-attaches an IOMMU
	 * domain from DT iommus= - maps into that domain and performs the
	 * needed cache sync as part of the normal DMA-API map. No separate
	 * ion_iovmm_map()/exynos_ion_sync_dmabuf_for_device() step needed.
	 */
	...
	if (!dma->dma_addr || IS_ERR_VALUE(dma->dma_addr)) {
		decon_err("dma_buf_map_attachment() gave no dma_addr: %pa\n", &dma->dma_addr);
		goto err_iovmm_map;
	}
		...
		struct decon_win_config *win_config, struct decon_reg_data *regs)
{
	...
	for (i = 0; i < plane_cnt; ++i) {
		...
		if (win_config->protection) {
			cfw_buf_list_create(dma_buf_data[i].sg_table,
					regs, vpp_id);
		}
	...
	if (win_config->fence_fd >= 0) {
		dma_buf_data[0].fence = sync_file_get_fence(win_config->fence_fd);
...
void decon_vpp_wait_wb_framedone(struct decon_device *decon)
{
	...
	if (v4l2_subdev_call(sd, core, ioctl,
		...
		decon_warn("%s: timeout of framedone (%lld)\n",
				__func__,
				atomic64_read(&decon->fence_seqno));
		decon_warn("\t\t eclk(%lu)\n",
		...
		struct decon_reg_data *regs)
{
	...
	for (i = 0; i < decon->pdata->max_win; i++) {
		...
		if (decon->vpp_usage_bitmask & (1 << win->vpp_id)) {
			...
			/*
			 * decon->disp_qos/int_qos: see the dev_pm_qos
			 * migration note on bts_add_request() in
			 * decon_bts.c - global PM_QOS_DISPLAY_THROUGHPUT/
			 * PM_QOS_DEVICE_THROUGHPUT are gone from mainline.
			 */
		}
	}
}
...
void decon_fence_err_log(struct decon_device *decon, int idx, struct dma_fence *fence)

{
	...
	if (decon->fence_err_cnt == 0) {
		...
		decon->first_fence_err.status = status;
		strscpy(decon->first_fence_err.name, name,
			sizeof(decon->first_fence_err.name));
	}
	...
	decon->fence_err_list[err_idx].status = status;
	strscpy(decon->fence_err_list[err_idx].name, name,
		sizeof(decon->fence_err_list[err_idx].name));
...
static int decon_fence_wait(struct dma_fence *fence)
{
	long ret = dma_fence_wait_timeout(fence, false, msecs_to_jiffies(900));
	if (ret == 0) {
		decon_warn("error waiting on acquire fence: timeout\n");
		return -ETIME;
	} else if (ret < 0) {
		decon_warn("error waiting on acquire fence: %ld\n", ret);
		return ret;
	}
	return 0;
}
...
/*
 * Retire fence: replaces the old sw_sync-timeline-backed
 * sw_sync_pt_create()+sync_fence_create()+sync_fence_install() triple.
 * mainline dropped the in-kernel sw_sync API entirely, so this uses a
 * plain dma_fence on decon's own fence_context/fence_seqno pair, wrapped
 * in a sync_file the same way modern DRM/V4L2 drivers hand retire/release
 * fences back to userspace.
 */
...
static const char *decon_fence_get_timeline_name(struct dma_fence *fence)
{
	return "decon-retire";
}

static const struct dma_fence_ops decon_fence_ops = {
	.get_driver_name = decon_fence_get_driver_name,
	.get_timeline_name = decon_fence_get_timeline_name,
};

static struct dma_fence *decon_create_retire_fence(struct decon_device *decon)
{
	...
	dma_fence_init(fence, &decon_fence_ops, &decon->fence_lock,
			decon->fence_context,
			atomic64_inc_return(&decon->fence_seqno));
	return fence;
}

/*
 * Wraps @fence in a sync_file and installs it at @fd, matching what
 * sync_fence_install() used to do. Returns 0 on success; on failure @fd
 * is left untouched and the caller still owns @fence.
 */
static int decon_install_retire_fence(struct dma_fence *fence, int fd)
{
...
static void decon_update_regs(struct decon_device *decon, struct decon_reg_data *regs)
{
	...
	if (decon->pdata->out_type == DECON_OUT_WB) {
		...
		decon->vpp_usage_bitmask = 0;
		decon_set_protected_content(decon, NULL);
		decon_dbg("write-back fence seqno:%lld\n",
				atomic64_read(&decon->fence_seqno));
	} else {
	...
	if (regs->retire_fence) {
		dma_fence_signal(regs->retire_fence);
		dma_fence_put(regs->retire_fence);
		regs->retire_fence = NULL;
	}
		...
		struct decon_win_config_data *win_data)
{
	...
	if ((decon->state == DECON_STATE_OFF) || (decon->ignore_vsync == true) ||
		(decon->pdata->out_type == DECON_OUT_TUI)) {
		fence = decon_create_retire_fence(decon);
		if (fence) {
			dma_fence_signal(fence);
			if (!decon_install_retire_fence(fence, fd)) {
				win_data->fence = fd;
				dma_fence_put(fence);
				goto err;
			}
...
windows_config:
	...
	if (ret) {
		...
		if (regs->num_of_window) {
			fence = decon_create_retire_fence(decon);
			if (fence && !decon_install_retire_fence(fence, fd)) {
				win_data->fence = fd;
				/* handed to decon_update_regs() below, which
				 * signals+puts it once this regs is retired */
				regs->retire_fence = fence;
			} else {
				if (fence)
					dma_fence_put(fence);
				put_unused_fd(fd);
...
static int decon_s_fmt(struct v4l2_subdev *sd, struct v4l2_subdev_state *state,
...
static int decon_register_entity(struct decon_device *decon)
{
	...
	if (ret)
		/* subdev was already registered above - undo it, or a
		 * deferred probe retry re-registers an entity that's
		 * still live in the media graph */
...
static int decon_fb_alloc_memory(struct decon_device *decon, struct decon_win *win)
{
	...
	/*
	 * ION is gone from mainline; this is window 0's own backing memory
	 * for the classic fbdev mmap path (distinct from the dma-buf-fd
	 * DECON_WIN_STATE_BUFFER path in decon_set_win_buffer()), so it
	 * doesn't need to be an importable/exportable dma-buf at all - a
	 * plain coherent DMA allocation is what vendor's own non-ION board
	 * fallback already did here.
	 */
	fbi->screen_base = dma_alloc_wc(decon->dev, size, &map_dma, GFP_KERNEL);
	if (!fbi->screen_base)
		return -ENOMEM;
...
static int decon_acquire_windows(struct decon_device *decon, int idx)
{
	...
	fbinfo->fbops		= &decon_fb_ops;
	/* FBINFO_FLAG_DEFAULT was vendor's own fb.h fork's name for "no
	 * flags", i.e. 0 - never existed upstream. */
	fbinfo->flags		= 0;
...
static int decon_probe(struct platform_device *pdev)
{
	...
	decon->fence_context = dma_fence_context_alloc(1);
	atomic64_set(&decon->fence_seqno, 0);
	spin_lock_init(&decon->fence_lock);
