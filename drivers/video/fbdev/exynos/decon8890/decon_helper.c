...
/*
 * struct timeval/decon_ktime_to_tv() were dropped in the Y2038-safe time
 * API cleanup. This is only ever used to pretty-print a ktime_t as
 * seconds.microseconds for the event-log debugfs dump below, so a small
 * local replacement is simpler than pulling in struct timespec64 at
 * every call site.
 */
struct decon_tv {
	long tv_sec;
...
static struct decon_tv decon_ktime_to_tv(ktime_t kt)
{
	...
	do {
		...
		case DISP_EVT_TE_INTERRUPT:
			...
			seq_printf(s, "time_diff=[%ld.%04lds]\n",
					decon_ktime_to_tv(prev_ktime).tv_sec,
					decon_ktime_to_tv(prev_ktime).tv_usec/100);
		...
		case DISP_EVT_ENTER_LPD:
			...
			tv = decon_ktime_to_tv(log->data.pm.elapsed);
