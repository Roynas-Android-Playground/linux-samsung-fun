...
void dsim_cmd_fail_detector(struct timer_list *t)
{
	struct dsim_device *dsim = timer_container_of(dsim, t, cmd_timer);
	struct decon_device *decon = (struct decon_device *)dsim->decon;
...
static int dsim_probe(struct platform_device *pdev)
{
	...
	/* see decon_f_register_irq() for why platform_get_irq(), not
