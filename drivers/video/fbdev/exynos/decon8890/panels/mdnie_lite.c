...
/*
 * Mainline dropped FB_EVENT_BLANK (only FB_EVENT_FB_REGISTERED/
 * UNREGISTERED remain in fb_notifier_call_chain(), see fbmem.c) so this
 * can no longer learn of blank/unblank via the fb notifier chain. decon
 * core already pokes the mdnie hw registers directly on blank/unblank
 * (mdnie_reg_start()/mdnie_reg_stop() in decon_core.c); it needs to also
 * call mdnie_update()/toggle mdnie->enable here directly for the
 * scenario/tuning-table push this used to do. Not wired up yet.
 */
