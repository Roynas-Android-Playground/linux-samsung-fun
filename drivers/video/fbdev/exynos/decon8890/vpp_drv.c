...
void vpp_op_timer_handler(struct timer_list *t)
{
	struct vpp_dev *vpp = timer_container_of(vpp, t, op_timer);
...
/*
 * SysMMU prefetch-buffer read-ahead direction tuning
 * (SYSMMU_PBUFCFG_... and sysmmu_set_prefetch_buffer_property()) is a
 * pure perf optimization for streaming reads on rotated buffers, and
 * it is vendor-only: mainline's drivers/iommu/exynos-iommu.c has no
 * equivalent API at all (not a rename - the feature genuinely isn't
 * there). Not porting that driver-level addition for a display port;
 * no-op for now, so VPP just always uses the SysMMU's default prefetch
 * behavior.
 */
...
void vpp_set_deadlock_time(struct vpp_dev *vpp, int msec)
{
	...
	/* Use the functional CCF clock, not the removed PWRCAL DISP cache. */
	rate_khz = clk_get_rate(vpp->res.gate) / KHZ;
	deadlock_num = msec * rate_khz;
	...
	if (test_bit(VPP_POWER_ON, &vpp->state)) {
		...
		if (is_err_irq(vpp_irq)) {
			...
			if (!(vpp_irq == VG_IRQ_DEADLOCK_STATUS)) {
				/*
				 * vendor also dumped SYSMMU fault status here
				 * (exynos_sysmmu_show_status()) - mainline's
				 * exynos-iommu.c has no equivalent exported
				 * debug dump; diagnostic-only, dropped rather
				 * than faked.
				 */
				goto err;
			}
...
static int vpp_probe(struct platform_device *pdev)
{
	...
	/*
	 * No iovmm_activate() equivalent needed: mainline's IOMMU-DMA glue
	 * (see drivers/iommu/exynos-iommu.c, CONFIG_EXYNOS_IOMMU) attaches
	 * this device's default domain automatically from its DT "iommus="
	 * property at probe time - vendor's manual activate/deactivate pair
	 * managed what of_iommu_configure() now does for us.
	 */

	timer_setup(&vpp->op_timer, vpp_op_timer_handler, 0);

	/*
	 * vendor also held a struct pm_qos_request here (PM_QOS_BUS_THROUGHPUT,
	 * floor 0) that's never updated anywhere after probe - real VPP
	 * bus/MIF bandwidth management goes through vpp->bts_ops above
	 * (decon_bts_control), and the global PM_QOS_BUS_THROUGHPUT class
	 * this used doesn't exist in mainline's pm_qos anymore. Dropped as
	 * dead weight rather than given a fake replacement.
	 */
	...
	/*
	 * vendor registered a per-domain fault handler here
	 * (vpp_sysmmu_fault_handler) via iommu_set_fault_handler(). Dropped:
	 * exynos_sysmmu_irq() never consults domain->handler at all (it
	 * panics unconditionally on a real fault, by design - see the
	 * memory-region fix for the actual bug this used to paper over), and
	 * modern per-device default domains are owned by the DMA-IOMMU API
	 * cookie, so this call now just WARN_ONs and no-ops.
	 */
